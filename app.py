...
def check_format(df, column_formats, file_key):
    ...
    if missing_columns:
        logger.error(f"Missing columns: {', '.join(missing_columns)} in {file_key} dataset")
        return False
    ...
    for column, expected_type in column_formats.items():
        ...
        if "datetime64" in expected_type:
            if not pd.api.types.is_datetime64_any_dtype(df[column]):
                logger.error(
                    f"Format check failed for column {column}: expected {expected_type}, "
                    f"found {actual_dtype} in {file_key} dataset"
                )
                return False
        else:
            if str(actual_dtype) != expected_type:
                logger.error(
                    f"Format check failed for column {column}: expected {expected_type}, "
                    f"found {actual_dtype} in {file_key} dataset"
                )
