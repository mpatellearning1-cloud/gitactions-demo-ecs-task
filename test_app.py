...
def s3(aws_credentials):
    with mock_aws():
        ...
        s3.create_bucket(Bucket='mpatel-learning-labs')
        csv_content = "id,order_id,user_id,product_id,created_at\n1,100,200,300,2021-06-01T00:00:00Z"
        s3.put_object(Bucket='mpatel-learning-labs', Key='ecommerce-data/new/orders/test.csv', Body=csv_content)
        yield s3
...
def test_list_files(s3):
    files = app.list_files('mpatel-learning-labs', 'ecommerce-data/new/orders')
    assert files == ['ecommerce-data/new/orders/test.csv'], "File list is incorrect"

def test_read_from_s3(s3):
    df = app.read_from_s3('mpatel-learning-labs', 'ecommerce-data/new/orders/test.csv')
